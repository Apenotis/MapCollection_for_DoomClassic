...
echo %G%* Scanne %B%%INSTALL_DIR%%W% Ordner nach neuen Daten...%W%
...
if "%zip_found%"=="1" (
    echo %G%* Archive erkannt.%W%
    echo   %GRA%Starte Entpackungsvorgang...%W%
...
echo %G%* Analysiere Strukturen und Metadaten...%W%
echo.
...
for /d %%D in ("%INSTALL_DIR%\*") do (
    ...
    for %%F in ("%%~fD\*.txt") do (
        echo   %GRA%-- Scane Textdatei: %%~nxF%W%
        if "!m_name!"=="" (
    ...
    if not "!m_name!"=="" echo     %G%Name - %W%!m_name!
    if not "!m_iwad!"=="" echo     %G%IWAD - %W%!m_iwad!

    findstr /C:",!m_name!," "%CSV_FILE%" >nul 2>&1
    if !errorlevel! EQU 0 (
        echo   %Y%-- Karte bereits vorhanden. Ueberspringe...%W%
    ) else (
        ...
        echo   %G%-- Erfolg: ID: %CY%!id!%W%  IWAD: %CY%!m_iwad!%W%  Pfad: %CY%!m_fold!\%W%
    )
    rd /s /q "%%~fD"
    echo.
)
...
echo %B%------------------------------------------------------%W%
echo %G%INSTALLATION ERFOLGREICH BEENDET.%W%
echo %B%------------------------------------------------------%W%
pause
exit /b

:manual_selector
echo   %R%-- IWAD nicht erkannt fuer: %CY%%~1%W%
echo   -- Manuell: 1:Doom1  2:Doom2  3:Plutonia  4:TNT
set "choice=2"
